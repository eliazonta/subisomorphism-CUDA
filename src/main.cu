...
int main(int argc, char** argv) {
    ...
    // CUDA kernel 
    // myCoolSubgraphIsomorhismKernel<<<grid_size, block_size>>>(target, pattern, mapping, found, patternVert, targetVert);
    
    // cuda_check_status(cudaDeviceSyncronize());

    // Copy the results (valid mappings) 
    #ifdef USE_STRUCT
    #endif
